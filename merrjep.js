...
var domain = wind.location.hostname;
...
function initScript() {
	...
	var path = wind.location.pathname;
	var url = decodeURIComponent(window.location.href);

	// Confirm page
	if (path.toLowerCase().indexOf('konfirmimi') > -1) {
		closePage();
	}

	// Renewal page
	else if (path.toLowerCase().indexOf('rregullim-i-shpalljes') > -1) {
		renewalPost();
	}
